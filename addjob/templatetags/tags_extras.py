...
from addjob.models import Stanowisko, Branza
...
def items_inside(category):
    count = Stanowisko.objects.filter(Q(branza=category.id) | Q(sektor=category.id)).count()
    return count

def first_letter(object):
    try:
        l = []
        for item in object:
            word = str(item)
